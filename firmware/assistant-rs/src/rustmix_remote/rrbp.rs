...
impl RrbpPacket {
    ...
    pub fn event(self) -> Option<RemoteEvent> {
        match self.command {
...
mod tests {
    ...
    fn drops_duplicate_sequence() {
        let mut parser = RrbpParser::new();
        assert_eq!(parser.parse(&[0x01, 0x07, 0x01, 0x00, 0x00, 0x00]).unwrap(), Some(RemoteEvent::PageNext));
        assert_eq!(parser.parse(&[0x01, 0x07, 0x02, 0x00, 0x00, 0x00]).unwrap(), None);
    }

    #[test]
    fn ignores_unknown_command() {
        let mut parser = RrbpParser::new();
        assert_eq!(parser.parse(&[0x01, 0x02, 0xFF, 0x00, 0x00, 0x00]).unwrap(), None);
    }

    #[test]
    fn rejects_bad_version() {
        let mut parser = RrbpParser::new();
        assert_eq!(parser.parse(&[0x02, 0x00, 0x01, 0x00, 0x00, 0x00]), Err(RrbpError::UnsupportedVersion { got: 0x02 }));
    }

    #[test]
    fn rejects_short_packet() {
        let mut parser = RrbpParser::new();
        assert_eq!(parser.parse(&[0x01, 0x00, 0x01]), Err(RrbpError::PacketTooShort { got: 3 }));
    }

    #[test]
    fn rejects_long_packet() {
        let mut parser = RrbpParser::new();
        assert_eq!(parser.parse(&[0x01, 0x00, 0x01, 0, 0, 0, 0]), Err(RrbpError::PacketTooLong { got: 7 }));
    }
}
