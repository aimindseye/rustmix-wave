...
use crate::rustmix_x4::x4_kernel::ui::{free_stack_bytes, stack_high_water_mark};
...
impl super::Kernel {
    ...
    async fn render<A: AppLayer>(&mut self, app_mgr: &mut A, mut redraw: Redraw) -> bool {
        ...
        'render: {
            ...
            if matches!(redraw, Redraw::Full | Redraw::Partial(_)) {
                let loading_active_before_full_update = app_mgr.ctx_mut().loading_active();
                if matches!(redraw, Redraw::Full) {
                    {
                        let mut handle = self.handle();
                        app_mgr.pre_render_full_redraw(&mut handle).await;
                    }

                    match app_mgr.take_redraw() {
                        Redraw::None => {}
                ...
                // Single-pass transition redraw fix. Background work can queue a
                // dirty redraw while the EPD is busy with the full-update waveform.
                // For transition full frames, the final frame has already been
                // written before the waveform starts, so an immediate second
                // refresh produces a visible double-refresh with little value.
                // Drop that redraw here unless a deferred input transition is
                // about to be applied; the deferred transition will request its
                // own redraw below.
                let loading_cleared_during_full_update =
                    loading_active_before_full_update && !app_mgr.ctx_mut().loading_active();

                if matches!(redraw, Redraw::Full)
                    && deferred.is_none()
                    && app_mgr.has_redraw()
                    && !loading_cleared_during_full_update
                {
                    let dropped = app_mgr.take_redraw();
                    info!(
                        "display: single-pass transition dropped redraw queued during full update: {:?}",
                        dropped
                    );
                }
    ...
    fn wait_for_power_button_release_before_sleep(&mut self) {
        ...
        if crate::rustmix_x4::x4_kernel::board::power_button_is_low() {
            log::info!("sleep: waiting for power button release before deep sleep");
        }

        while crate::rustmix_x4::x4_kernel::board::power_button_is_low() {
    ...
    fn sd_card_sleep(&self) {
        ...
        critical_section::with(|cs| {
            let bus_ref = crate::rustmix_x4::x4_kernel::board::SPI_BUS_REF
                .borrow(cs)
                .get();
            let mut cs_pin = crate::rustmix_x4::x4_kernel::board::SD_CS_SLEEP.borrow_ref_mut(cs);
