...
use crate::rustmix_x4::x4_kernel::board::action::ActionEvent;
...
use crate::rustmix_x4::x4_kernel::ui::Region;
...
pub trait AppLayer {
    ...
    // Optional pre-render hydration used only before a full-screen transition
    // redraw. Apps that can cheaply load list/status data before a full frame
    // is written may override this to avoid a loading-frame refresh followed by
    // an immediate second refresh for the loaded content. The default keeps
    // existing behavior for apps with expensive or unsafe pre-render work.
    async fn pre_render_full_redraw(&mut self, _k: &mut KernelHandle<'_>) {}
